...
(function () {
  ...
  // sobre la azul y roja. Si algún día entra una bandera nueva, esto es lo que
  // hay que decidirle.
  var BASE = "https://j-mamanche.github.io/tratratrax-web/media/banderas/";
  var BANDERAS = [
    { archivo: "estrellas.svg",   tinta: "black" },  // amarillo
    { archivo: "sur.svg",         tinta: "black" },  // verde claro
    { archivo: "diagonal.svg",    tinta: "black" },  // gris y amarillo
    { archivo: "de-colombia.svg", tinta: "white" }   // azul y rojo
  ];
  ...
  function mostrar() {
    ...
    } while (imgs.length > 1 && k === ultima);
    ultima = k;

    for (var j = 0; j < imgs.length; j++) imgs[j].style.opacity = j === k ? "1" : "0";
  ...
  function montar() {
    ...
    if (capa.parentNode !== document.body) document.body.appendChild(capa);
...
resto del material, como los emblemas del About. Son SVG de `1440×1024`, así
que escalan a cualquier pantalla sin pesar nada:

| Archivo | Qué es |
|---|---|
| `estrellas.svg` | estrellas sobre amarillo |
| `sur.svg` | `SUR` sobre verde, con franjas roja y negra |
| `diagonal.svg` | `TRATRRRRRATRAX` sobre el diagonal amarillo/gris con estrella |
| `de-colombia.svg` | `DE COLOMBIA / FROM COLOMBIA / DE COLOMBIA`, azul-rojo-azul |

Van **a sangre** (`object-fit: cover`), así que en una ventana muy apaisada o muy
alta se recortan por el lado largo. Es a propósito: una bandera con franjas
blancas a los lados no es una bandera.

Para cambiar el set: se sobreescriben los archivos y se toca la lista
`BANDERAS` del script. Nada más.
